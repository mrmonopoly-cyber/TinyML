...
let yparse syntax_error parser (tokenizer: Lexing.LexBuffer<_> -> 'tok) tokenTagToTokenId =
    let pretty_token_by_tags =
        ...
        fun ns ->
            ...
            if List.length ns > 1 then sprintf "one among %s" r else r
    ...
    let tokenizer lexbuf =
        let tok =
            try
                tokenizer lexbuf
            with e ->
                raise (syntax_error (e.Message, lexbuf))
    ...
    fun (lexbuf: Lexing.LexBuffer<_>) ->
        ...
        with LexYacc.ParseErrorContextException ctx as e ->
            ...
            let msg =
                sprintf
                    "encountered token %s but expected %s%s"
                    tok
                    (pretty_token_by_tags ctx.ShiftTokens)
                    (let l = ctx.ReduceTokens in

                     if l.IsEmpty then
                         ""
                     else
                         sprintf ", or alternatively %s" (pretty_token_by_tags l)) in
